...
plt.xticks(np.arange(0, max(data['1kU Pricing(USD)'])+1, 1250))
plt.show()   
...
## filters the prediction model such that 0 or less values are not considered as they are invalid
while prediction <= 0:   
    prediction = model.predict(features)
print(prediction)  


clf = MLPClassifier(solver='lbfgs', alpha=1e-5, hidden_layer_sizes=(15, 10), random_state=42, batch_size = 300) 
clf.fit(data[['# of CPU Cores', '# of Threads', 'Base Clock(GHz)', 'L1 Cache (KB)', 'L2 Cache(MB)', 'L3 Cache(MB)']].values, data["1kU Pricing(USD)"].values)   
MLPClassifier(solver='lbfgs', alpha=1e-5, hidden_layer_sizes=(42, 10), random_state=42, batch_size = 300)
